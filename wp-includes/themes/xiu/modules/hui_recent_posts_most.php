    ...
    if( !_hui('most_list_style') || _hui('most_list_style')=='comment' ){
        ...
        if($most_viewed) {
            ...
            foreach ($most_viewed as $post) {
                ...
                $output .= '<li><p class="text-muted"><span class="post-comments">'.__('阅读', 'haoui').' ('.$post_views.')</span>'.hui_get_post_like($class='post-like', $pid=$post->ID).'</p><span class="label label-'.$i.'">'.$i.'</span><a'.hui_target_blank().' href="'.get_permalink($post->ID).'" title="'.$title.'">'.$title.'</a></li>';
                $i++;
            }
        } else {
        ...
        if($most_viewed) {
            ...
            foreach ($most_viewed as $post) {
                ...
                // $output .= '<li class="item-'.$i.'"><a target="_blank" href="'.get_permalink($postid).'">'._get_post_thumbnail(array()).'<h2>'.$post_title.'</h2><p>'.hui_get_post_date( get_the_time('Y-m-d H:i:s') ).'<span class="post-views">阅读('.$post_views.')</span></p></a></li>';
                $output .= '<li><p class="text-muted">'.hui_get_post_like($class='post-like', $pid=$post->ID).'</p><span class="label label-'.$i.'">'.$i.'</span><a'.hui_target_blank().' href="'.get_permalink($post->ID).'" title="'.$title.'">'.$title.'</a></li>';
                $i++;
            }
        } else {
            $output = '<li>'.__('暂无文章！', 'haoui').'</li>';
